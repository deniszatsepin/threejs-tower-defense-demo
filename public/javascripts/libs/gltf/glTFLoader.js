...
(function(root, factory) {
	if (typeof define === 'function' && define.amd) {
		define(['three', './gltf/glTF-parser', './gltf/glTFLoaderUtils'], function(THREE, glTFParser, GLTFLoaderUtils) {
			return factory(root, THREE, glTFParser, GLTFLoaderUtils);
		})
	} else {
		factory(root);
	}
}(this, function(root, THREE, glTFParser, GLTFLoaderUtils) {
	...
	THREE.glTFLoader.prototype.load = function (url, callback) {
		...
		function componentsPerElementForGLType(glType, type) {
			var size = 0;
			switch (glType) {
			...
			switch (type) {
				...
				case 'MAT4':
					return size * 16;
				default:
					console.warn('[conponentsPerElementForGLType] Undefined type');
					return size;
			}
		}
		...
		VertexAttributeDelegate.prototype.bufferResourceAvailable = function (glResource, ctx) {
			...
			if (semantic == "POSITION") {
				// TODO: Should be easy to take strides into account here
				nComponents = componentsPerElementForGLType(attribute.componentType, attribute.type);
				floatArray = new Float32Array(glResource, 0, attribute.count * nComponents);
...
//               		texture.minFilter = THREE.LinearFilter;
					...
					var normalMap = values.bump;
					var normalEntry = null;
					var normalPath = null, normalParams = null;
					if (normalMap) {
						normalEntry = this.resources.getEntry(normalMap);
						if (normalEntry) {
							var imageEntry = this.resources.getEntry(normalEntry.description.source);
							if (imageEntry) {
								normalPath = imageEntry.description.path;
							}
							var samplerEntry = this.resources.getEntry(normalEntry.description.sampler);
							if (samplerEntry) {
								normalParams = samplerEntry.description;
							}
						}
					}
					var normal = LoadTexture(normalPath);
					if (normal && normalParams) {
						if (normalParams.wrapS == WebGLRenderingContext.REPEAT)
							normal.wrapS = THREE.RepeatWrapping;

						if (normalParams.wrapT == WebGLRenderingContext.REPEAT)
							normal.wrapT = THREE.RepeatWrapping;

						if (normalParams.magFilter == WebGLRenderingContext.LINEAR)
